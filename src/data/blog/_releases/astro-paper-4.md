...
title: LunarLight 4.0
slug: "astro-paper-v4"
featured: false
ogImage: ../../../assets/images/LunarLight-v4.png
tags:
  - release
description: "LunarLight v4: ensuring a smoother and more feature-rich blogging experience."
---

Hello everyone! Wishing you a happy New Year 🎉 and all the best for 2024! We're excited to announce the release of LunarLight v4, a significant update that introduces a range of new features, improvements, and bug fixes to elevate your blogging experience. A big thank you to all the contributors for their valuable input and efforts in making version 4 possible!

![LunarLight v4](@/assets/images/LunarLight-v4.png)
...
LunarLight now leverages the power and capabilities of Astro v4. However, it’s a subtle upgrade and won’t break most Astro users.
...
The behavior of the `slug` is slightly different now. In the previous versions of LunarLight, if the `postSlug` is not specified in a blog post (markdown file), the title of that blog post would be slugified and used as the `slug`. However, in LunarLight v4, if the `slug` field is not specified, the markdown file name will be used as the `slug`. One thing to keep in mind is that the `slug` field can be omitted, but it cannot be an empty string (slug: "" ❌).

If you're upgrading LunarLight from v3 to v4, make sure to replace `postSlug` in your `src/content/blog/*.md` files with `slug`.

## New Features

### Add code-snippets for content creation [#206](https://github.com/satnaing/astro-paper/pull/206)

LunarLight now includes VSCode snippets for new blog posts, eliminating the need for manual copy/pasting of the frontmatter and content structure (table of contents, heading, excerpt, etc.).
...
![Last Modified Date feature in LunarLight](https://github.com/satnaing/astro-paper/assets/53733092/cc89585e-148e-444d-9da1-0d496e867175)
...
![Back to top button in LunarLight](https://github.com/satnaing/astro-paper/assets/53733092/79854957-7877-4f19-936e-ad994b772074)
...
LunarLight v4 now dynamically generates the robots.txt file, giving you more control over search engine indexing and web crawling. Besides, sitemap URL will also be added inside `robot.txt` file.

### Add Docker-Compose File [#174](https://github.com/satnaing/astro-paper/pull/174)

Managing your LunarLight environment is now easier than ever with the addition of a Docker-Compose file, simplifying deployment and configuration.
...
We believe these updates will significantly elevate your LunarLight experience. Thank you to everyone who contributed, solved issues, and gave stars to LunarLight. We look forward to seeing the amazing content you create with LunarLight v4!

Happy Blogging!

[Sat Naing](https://satnaing.dev) <br/>
Creator of LunarLight
